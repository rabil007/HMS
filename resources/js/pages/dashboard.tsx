...
export default function Dashboard() {
    ...
    return (
        <div className="relative min-h-screen w-full bg-[#080C15] text-white overflow-hidden flex flex-col font-sans">
            <Head title="Dashboard" />

            {/* Ambient blobs */}
            <div className="absolute top-[-15%] left-[25%] w-[40rem] h-[40rem] bg-blue-900/15 rounded-full blur-[120px] pointer-events-none" />
            <div className="absolute bottom-[-10%] right-[20%] w-[35rem] h-[35rem] bg-indigo-900/15 rounded-full blur-[100px] pointer-events-none" />

            {/* Header */}
            <header className="relative z-20 flex items-center justify-between px-5 sm:px-8 py-4 border-b border-white/[0.06]">
                <Link href={toUrl(dashboard())} className="select-none">
                    <AppLogoIcon className="h-9 w-auto" />
                </Link>

                <DropdownMenu>
                    <DropdownMenuTrigger className="focus:outline-none flex items-center gap-2.5 rounded-full px-2 py-1.5 hover:bg-white/6 border border-transparent hover:border-white/10 transition-all cursor-pointer">
                        <span className="text-[13px] font-medium text-zinc-300 hidden sm:block">{user.name}</span>
                        ...
                        >
                            ...
                            <div
                                className={[
                                    ...
                                    'group-focus-visible:ring-4 group-focus-visible:ring-white/30 group-focus-visible:ring-offset-4 group-focus-visible:ring-offset-[#080C15]',
                                ].join(' ')}
                            >
                            ...
                            {/* Label */}
                            <span className="text-[12px] sm:text-[13px] font-semibold text-zinc-400 group-hover:text-white transition-colors text-center tracking-wide">
                                {module.name}
                            </span>
                        </Link>
                    ))}
