...
export function GuestImportModal({ open, onOpenChange }: GuestImportModalProps) {
    const previewUrl = toUrl(importPreview());
    const importUrl = toUrl(importGuests());
    const templateUrl = toUrl(importTemplate());
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            try {
                ...
                // Non-JSON response body (e.g. server/proxy HTML error page).
            }
            ...
            if (!res.ok) {
                const message =
                    ...
                    (res.status === 413
                        ? 'File is too large for server upload limits. Please use CSV or a smaller XLSX file.'
                        : `Failed to read file (HTTP ${res.status}).`);
            ...
            if (rows.length === 0) {
                setFileError('No valid guests found in the file. Name column is required.');

                return;
            }

            setPreviewRows(rows);
            setSelectedIndices(new Set(rows.map((row, i) => (!row.isDuplicate ? i : -1)).filter((i) => i >= 0)));
            setStep('preview');
            setActiveTab('all');
        } catch {
    ...
    const visibleRows = previewRows.reduce<{ row: PreviewRow; globalIndex: number }[]>((acc, row, index) => {
        const matches = activeTab === 'all' || (activeTab === 'new' && !row.isDuplicate) || (activeTab === 'duplicates' && row.isDuplicate);

        if (matches) {
            acc.push({ row, globalIndex: index });
        }
    ...
    const handleImport = () => {
        ...
        if (rows.length === 0) {
            setImportError('Please select at least one guest to import.');
        ...
        router.post(importUrl, { rows }, {
            preserveScroll: true,
            onSuccess: () => handleClose(),
            onError: (errors) => {
                setImportError(errors.rows ?? errors['rows.0.name'] ?? 'Import failed. Please try again.');
                setImporting(false);
            },
        ...
        <Dialog open={open} onOpenChange={(val) => (val ? onOpenChange(true) : handleClose())}>
            <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
                <DialogHeader className="shrink-0">
                    ...
                    <DialogDescription>
                        {step === 'upload'
                            ? 'Upload an Excel or CSV file with Name (required), Email (optional), and Phone (optional).'
                            : `${previewRows.length} guests found. Review and select the ones you want to import.`}
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="space-y-4 py-2 overflow-y-auto">
                        <div className="flex items-center justify-between p-3 border rounded-lg bg-muted/20">
                            <div className="text-sm">
                                ...
                                <p className="text-muted-foreground text-xs mt-0.5">Download the guest template (Name, Email, Phone).</p>
                            </div>
                    ...
                    <div className="flex flex-col min-h-0 flex-1 py-2 gap-3">
                        ...
                        <div className="flex gap-1 p-1 bg-muted/40 rounded-lg shrink-0">
                            {tabs.map((tab) => (
                                ...
                                >
                                    {tab.label}
                                    <span className="text-[11px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{tab.count}</span>
                                </button>
                            ))}
                        </div>

                        <div className="flex items-center justify-between px-4 py-2.5 bg-muted/20 rounded-lg border border-border/40 shrink-0">
                            <button type="button" className="flex items-center gap-2 text-sm font-medium hover:text-primary transition-colors" onClick={toggleAllVisible}>
                                {allVisibleSelected ? <CheckSquare2 className="size-4 text-primary" /> : <Square className="size-4 text-muted-foreground" />}
                                    ...
                                    visibleRows.map(({ row, globalIndex }) => {
                                        ...
                                        return (
                                            <button
                                                key={globalIndex}
                                                type="button"
                                                className={`w-full flex items-start gap-3 px-4 py-3 text-left text-sm transition-colors border-b border-border/30 last:border-0 hover:bg-muted/20 ${
                                                    checked ? 'bg-primary/5' : ''
                                                }`}
                                                onClick={() => toggleRow(globalIndex)}
                                            >
                                                {checked ? <CheckSquare2 className="size-4 text-primary mt-0.5 shrink-0" /> : <Square className="size-4 text-muted-foreground mt-0.5 shrink-0" />}
                                                <div className="flex-1 min-w-0">
                                                    <p className={`text-[13px] ${checked ? 'font-medium text-foreground' : 'text-muted-foreground'}`}>{row.name}</p>
                        ...
                        <>
                            ...
                            <Button onClick={handleImport} disabled={selectedIndices.size === 0 || importing}>
                                {importing ? (
                                    ...
                                    `Import ${selectedIndices.size} Guests`
                                )}
                            </Button>
                        </>
                    )}
