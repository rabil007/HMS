import { Head, Link, router } from '@inertiajs/react';
import { flexRender, getCoreRowModel, useReactTable } from '@tanstack/react-table';
import type { ColumnDef } from '@tanstack/react-table';
import { ArrowUpDown, Eye, Mail, Pencil, Phone, Plus, Trash2, Upload, User } from 'lucide-react';
import React from 'react';
import { ListSearch } from '@/components/list/list-search';
import { PaginationBar } from '@/components/list/pagination-bar';
import { RowsPerPageSelect } from '@/components/list/rows-per-page-select';
import { Button } from '@/components/ui/button';
import { useConfirmDialog } from '@/hooks/use-confirm-dialog';
import { useIndexQueryParams } from '@/hooks/use-index-query-params';
import PageLayout from '@/layouts/page-layout';
import { toUrl } from '@/lib/utils';
import { dashboard } from '@/routes';
import { create, destroy, edit, index as guestsIndex, show } from '@/routes/guests';
import { GuestImportModal } from './guest-import-modal';

type Paged<T> = {
    data: T[];
    links: { url: string | null; label: string; active: boolean }[];
    meta: { current_page: number; last_page: number; per_page: number; total: number };
};

type GuestRow = {
    id: number;
    full_name: string;
    email: string | null;
    phone: string | null;
    bookings_count: number;
};

export default function GuestsIndex({
    guests,
    filters,
}: {
    guests: Paged<GuestRow>;
    filters: { q?: string; sort?: string; dir?: 'asc' | 'desc'; per_page?: number };
}) {
    const { q, setQ, perPage, setPerPage, toggleSort } = useIndexQueryParams({
        href: guestsIndex(),
        filters,
        defaultPerPage: 15,
    });

    const slOffset = ((guests?.meta?.current_page ?? 1) - 1) * (guests?.meta?.per_page ?? 15);
    const { requestConfirm, ConfirmDialog } = useConfirmDialog();
    const [importModalOpen, setImportModalOpen] = React.useState(false);

    const columns = React.useMemo<ColumnDef<GuestRow>[]>(
        () => [
            {
                id: 'slno',
                header: () => <span>Sl No.</span>,
                cell: ({ row }) => <span className="text-[13px] font-semibold">{slOffset + row.index + 1}</span>,
            },
            {
                accessorKey: 'full_name',
                header: () => (
                    <button type="button" className="inline-flex items-center gap-2" onClick={() => toggleSort('full_name')}>
                        Name <ArrowUpDown className="size-4" />
                    </button>
                ),
                cell: ({ row }) => (
                    <span className="font-medium flex items-center gap-2">
                        <User className="size-3.5 text-muted-foreground" /> {row.original.full_name}
                    </span>
                ),
            },
            {
                accessorKey: 'email',
                header: () => <span>Email</span>,
                cell: ({ row }) =>
                    row.original.email ? (
                        <span className="flex items-center gap-2 text-muted-foreground">
                            <Mail className="size-3.5" /> {row.original.email}
                        </span>
                    ) : (
                        <span className="text-muted-foreground/60">—</span>
                    ),
            },
            {
                accessorKey: 'phone',
                header: () => <span>Phone</span>,
                cell: ({ row }) =>
                    row.original.phone ? (
                        <span className="flex items-center gap-2 text-muted-foreground">
                            <Phone className="size-3.5" /> {row.original.phone}
                        </span>
                    ) : (
                        <span className="text-muted-foreground/60">—</span>
                    ),
            },
            {
                accessorKey: 'bookings_count',
                header: () => <span>Bookings</span>,
                cell: ({ row }) => <span className="text-muted-foreground">{row.original.bookings_count}</span>,
            },
            {
                id: 'actions',
                header: () => <span>Actions</span>,
                cell: ({ row }) => (
                    <div className="flex items-center justify-end gap-1">
                        <Link
                            href={toUrl(show({ guest: row.original.id }))}
                            onClick={(e) => e.stopPropagation()}
                            className="flex items-center justify-center h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
                            title="View guest"
                        >
                            <Eye className="size-3.5" />
                        </Link>
                        <Link
                            href={toUrl(edit({ guest: row.original.id }))}
                            onClick={(e) => e.stopPropagation()}
                            className="flex items-center justify-center h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
                            title="Edit guest"
                        >
                            <Pencil className="size-3.5" />
                        </Link>
                        <button
                            type="button"
                            title="Delete guest"
                            onClick={async (e) => {
                                e.stopPropagation();

                                if (
                                    !(await requestConfirm({
                                        title: 'Delete this guest?',
                                        description: row.original.bookings_count > 0
                                            ? 'This guest is referenced by bookings and cannot be deleted.'
                                            : 'This action cannot be undone.',
                                        confirmText: 'Delete',
                                        variant: 'destructive',
                                    }))
                                ) {
                                    return;
                                }

                                router.delete(toUrl(destroy({ guest: row.original.id })), { preserveScroll: true });
                            }}
                            className="flex h-11 w-11 items-center justify-center rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
                        >
                            <Trash2 className="size-3.5" />
                        </button>
                    </div>
                ),
            },
        ],
        [slOffset, requestConfirm, toggleSort],
    );

    const table = useReactTable({
        data: guests.data,
        columns,
        getCoreRowModel: getCoreRowModel(),
        manualPagination: true,
        manualSorting: true,
        manualFiltering: true,
    });

    return (
        <PageLayout title="Guests" backHref={toUrl(dashboard())}>
            <ConfirmDialog />
            <Head title="Guests" />

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-xl font-bold text-foreground tracking-tight">Guests</h2>
                    <p className="text-[13px] text-muted-foreground mt-0.5">Master list of guest records used across bookings.</p>
                </div>

                <div className="flex items-center gap-2">
                    <Button variant="outline" className="rounded-full h-9 sm:h-10 px-4 text-[12px] sm:text-[14px]" onClick={() => setImportModalOpen(true)}>
                        <Upload className="size-3.5 sm:size-4 mr-1.5 sm:mr-2" /> Import
                    </Button>
                    <Button asChild className="rounded-full h-9 sm:h-10 px-4 text-[12px] sm:text-[14px]">
                        <Link href={toUrl(create())}>
                            <Plus className="size-3.5 sm:size-4 mr-1.5 sm:mr-2" /> New Guest
                        </Link>
                    </Button>
                </div>
            </div>

            <GuestImportModal open={importModalOpen} onOpenChange={setImportModalOpen} />

            <div className="mb-6">
                <ListSearch value={q} onChange={setQ} placeholder="Search by name, email, or phone…" />
            </div>

            <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden">
                <div className="w-full overflow-x-auto">
                    <table className="w-full min-w-max text-sm">
                        <thead className="bg-muted/30">
                            {table.getHeaderGroups().map((hg) => (
                                <tr key={hg.id} className="border-b border-border/40">
                                    {hg.headers.map((h) => (
                                        <th key={h.id} className="text-left px-4 py-3 font-semibold text-foreground whitespace-nowrap">
                                            {h.isPlaceholder ? null : flexRender(h.column.columnDef.header, h.getContext())}
                                        </th>
                                    ))}
                                </tr>
                            ))}
                        </thead>
                        <tbody>
                            {guests.data.length === 0 ? (
                                <tr className="border-b border-border/30 last:border-0">
                                    <td colSpan={columns.length} className="px-4 py-10 text-center">
                                        <p className="text-[15px] font-medium text-foreground">No guests yet</p>
                                        <p className="text-[13px] text-muted-foreground mt-1">Create your first guest to get started.</p>
                                    </td>
                                </tr>
                            ) : (
                                table.getRowModel().rows.map((row) => (
                                    <tr
                                        key={row.id}
                                        onClick={() => router.get(toUrl(show({ guest: row.original.id })))}
                                        className="border-b border-border/30 last:border-0 hover:bg-muted/20 cursor-pointer"
                                    >
                                        {row.getVisibleCells().map((cell) => (
                                            <td key={cell.id} className="px-4 py-3 align-middle whitespace-nowrap">
                                                {flexRender(cell.column.columnDef.cell, cell.getContext())}
                                            </td>
                                        ))}
                                    </tr>
                                ))
                            )}
                        </tbody>
                    </table>
                </div>
            </div>

            {guests.links?.length > 0 && (
                <PaginationBar
                    links={guests.links}
                    onVisit={(url) => router.get(url, {}, { preserveScroll: true, preserveState: true })}
                    left={<RowsPerPageSelect value={perPage} onChange={setPerPage} />}
                />
            )}
        </PageLayout>
    );
}

GuestsIndex.layout = (page: React.ReactNode) => page;
