...
import { index as hotelsIndex, update } from '@/routes/admin/hotels';

export default function HotelsEdit({ hotel }: { hotel: { id: number; name: string } }) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(toUrl(update({ hotel: hotel.id })));
    };

    return (
        <PageLayout title="Edit Hotel" backHref={toUrl(hotelsIndex())}>
            <Head title="Edit Hotel" />

            <FormPageHeader title="Edit Hotel" description="Update the hotel details." />
...
HotelsEdit.layout = (page: React.ReactNode) => page;
